    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallcoupon/skubounds/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.skuBounds.skuId
                this.dataForm.growBounds = data.skuBounds.growBounds
                this.dataForm.buyBounds = data.skuBounds.buyBounds
                this.dataForm.work = data.skuBounds.work
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallcoupon/skubounds/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
