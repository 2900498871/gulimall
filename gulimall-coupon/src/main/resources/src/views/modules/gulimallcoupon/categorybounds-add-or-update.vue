    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallcoupon/categorybounds/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.categoryId = data.categoryBounds.categoryId
                this.dataForm.growBounds = data.categoryBounds.growBounds
                this.dataForm.buyBounds = data.categoryBounds.buyBounds
                this.dataForm.work = data.categoryBounds.work
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/gulimallcoupon/categorybounds/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
